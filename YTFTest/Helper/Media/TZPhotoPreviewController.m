...
- (void)okButtonClick {
    ...
    if (self.okButtonClickBlock) {
        ...
        [self preViewFixedImage];
    }
    if (self.okButtonClickBlockWithPreviewType) {
    }
}
-(void)preViewFixedImage{
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:@"namemultipleImageChoice"  object:nil userInfo:@{@"imagePath":filePath,@"imageUrl":@"nil"}];
