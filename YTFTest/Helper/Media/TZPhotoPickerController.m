...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
    ...
    NSMutableArray *arrayPhoto;//所有选中的略缩图的容器
...
@property (nonatomic, strong) NSMutableArray *wholeFilePathsArray;
@property (nonatomic ,copy) NSString *wholeFilePath;
...
- (void)okButtonClick {
    ...
    if (!_wholeFilePathsArray) {
        _wholeFilePathsArray = [NSMutableArray array];
    }
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if (photo) {
                photo = [self scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];  //RUN
                [photos replaceObjectAtIndex:i withObject:photo];
                
                [arrayPhoto addObject:photo];
                
            }
            NSString *path = [AppDelegate shareAppDelegate].getFSPath;
            
           //生成随机字符串 创建不同文件夹名字
            NSString * randName = [self randomString];
            
            NSString *filePath = [path stringByAppendingPathComponent:[NSString stringWithFormat:@"savePhotoMany%@.png",randName]];   //通过取名字不同 从而 保存文件的
             [weakSelf.wholeFilePathsArray addObject:filePath];
            
             NSData *imageData = UIImagePNGRepresentation(photo);
            // 将Data 存入指定路径
             [imageData writeToFile:filePath atomically:YES];
            
            if (weakSelf.wholeFilePathsArray.count == tzImagePickerVc.selectedModels.count) {
                
                [[NSNotificationCenter defaultCenter]postNotificationName:@"namemultipleImageChoice"  object:nil userInfo:@{@"imagePath":self.wholeFilePathsArray,@"imageUrl":@"nil"}];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        ...
        TZAssetModel * targetModel = _models[index];
        
        CGSize  _imageSize = CGSizeMake(1000, 1000);
        ...
        [[PHImageManager defaultManager] requestImageForAsset:targetModel.asset targetSize:_imageSize contentMode:PHImageContentModeAspectFill options:requestOptions resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info){
        
            if (self.isCropImage) {
                
                            UIImage * targetImage = result;
                            PhotoTweaksViewController *photoTweaksViewController = [[PhotoTweaksViewController alloc] initWithImage:targetImage];
...
- (void)pushPhotoPrevireViewController:(TZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
    weakSelf.imagePickerVc.allowsEditing = YES;
