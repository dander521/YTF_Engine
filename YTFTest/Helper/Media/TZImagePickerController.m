...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate isCrop:(BOOL)isCrop{
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            [self pushToPhotoPickerVc:self.isCropImage];
        }
    }
    return self;
}
...
- (void)pushToPhotoPickerVc:(BOOL)isCrop{
    _pushToPhotoPickerVc = YES;
    if (_pushToPhotoPickerVc) {
        TZPhotoPickerController *photoPickerVc = [[TZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.isCropImage = isCrop;
