...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString *)photoUrl
{
    ...
    if (self) {
        //添加图片
        YTFSDWebImageManager *manager = [YTFSDWebImageManager sharedManager];
        ...
        if (!isCached) {//没有缓存
            ...
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:[UIImage imageNamed:@"photoBrowser"] options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize){
            ...
            UIImage *cachedImage = [[YTFSDImageCache sharedImageCache] imageFromDiskCacheForKey:photoUrl];
...
{
    [self.delegate longPressPhotoView:((UIImageView *)gestureRecongnizer.view).image];
}
